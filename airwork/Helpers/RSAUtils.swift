...
//        self.updatePublicKey()
    ...
    public func getMyPublicKeyString() -> String? {
        ...
        do {
            var data = try pubKey.data()
            return exportImportManager.exportRSAPublicKeyToPEM(data, keyType: kSecAttrKeyTypeRSA as String, keySize: RSAKeyManager.KEY_SIZE)
        } catch is Error {
            print("error setting pub key")
        }
